...
function getToolParams(name) {
  ...
  const schemas = {
    ...
    scan: { url: {type:'string'}, mode: {type:'string',enum:['active','passive']} },
    ...
    save_project: {},
    burp_version: {},
    add_issue: { name: {type:'string'}, url: {type:'string'}, detail: {type:'string'}, remediation: {type:'string'}, severity: {type:'string'}, confidence: {type:'string'} },
    ...
    websocket_create: { host: {type:'string'}, port: {type:'number'}, path: {type:'string'}, https: {type:'boolean'} },
  ...
  switch (method) {
    ...
    case 'tools/call': {
      ...
      if (!params || typeof params.name !== 'string') {
        return { jsonrpc: '2.0', id, error: { code: -32602, message: 'Invalid params: tools/call requires a tool name' } };
      }
