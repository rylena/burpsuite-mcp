...
# Requires: JDK 21+, curl (Node.js is optional and validates the MCP bridge)
...
mkdir -p "$LIB"
[ -f "$LIB/montoya-api.jar" ] || curl -fsSL -o "$LIB/montoya-api.jar" "$MONTOYA"
[ -f "$LIB/gson.jar" ]        || curl -fsSL -o "$LIB/gson.jar"        "$GSON"
[ -f "$LIB/nanohttpd.jar" ]   || curl -fsSL -o "$LIB/nanohttpd.jar"   "$NANO"
