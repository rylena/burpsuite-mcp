...
| 安全控制 | `scope_gate`, `privacy_mode`, `audit_log` |
...
### `scope_gate` / `privacy_mode` — 安全边界

- `scope_gate` 的 `action` 支持 `enable`、`disable`、`status`。启用后，Burp Proxy 会直接丢弃不在 Burp Target scope 内的请求，MCP 发起网络请求前也会检查同一 scope。
- `privacy_mode` 的 `mode` 支持 `strict`、`off`、`status`。`strict` 会强制启用 scope gate，并在 MCP 响应中隐藏认证头、Cookie、密码、token、API key 等敏感值。
- 先用 `add_to_scope` 或 Burp Target → Scope 配置授权目标，再启用这些控制。`audit_log` 可查看被拦截操作。

```json
{"tool": "scope_gate", "params": {"action": "enable"}}
```

```json
{"tool": "privacy_mode", "params": {"mode": "strict"}}
```
...
This project is source-available under the [PolyForm Noncommercial License 1.0.0](LICENSE.md). You may use, modify, and redistribute it for noncommercial purposes. Commercial use requires a separate license from the project owner.
